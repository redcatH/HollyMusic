/**
 * AI 协助创建歌单 - 分步全屏向导页
 * 移动端 fixed inset-0 全屏沉浸；桌面端 main 内居中卡片（md: 切换）。
 * framer-motion AnimatePresence 做方向感步骤切换。
 */
...
export function AiPlaylistPage() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-background md:relative md:z-auto md:h-full md:bg-transparent md:items-center md:justify-center">
      <div className="flex h-full w-full flex-col md:h-auto md:max-w-xl md:rounded-2xl md:border md:border-border md:bg-card md:shadow-2xl md:min-h-[600px] md:max-h-[88vh] md:overflow-hidden">
        {/* 顶部：关闭 + 进度（移动端 safe-area-top 避刘海） */}
        ...
        <div className="relative flex-1 overflow-hidden safe-area-bottom">
          <AnimatePresence custom={ai.direction} mode="wait">
            ...
            >
              ...
              {ai.step === 2 && (
                <StepProcessing processing={ai.processing} onRetry={ai.runProcess} />
              )}
              {ai.step === 3 && (
                <StepConfirm
